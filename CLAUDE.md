...
- **Duplicate-record ROOT CAUSE + final guard (2026-08-12).** Splits kept recurring after the 08-03 hardening (Lucky again, Hightown, The Mandalorian 17/7 — merged same day). Diagnosed: `FSEventLibraryUpdatesEnabled` (auto-scan on file change) raced the stereo batch's per-file rewrites — 4 workers rewriting one show's files → concurrent partial scans → second show record (Mandalorian's dup held exactly the 7 files rewritten Aug 7). **FSEvents now OFF**; coverage via Sonarr/Radarr Connect scans + hourly scheduled scan + newscan's own end-of-run refresh. Also: Agregarr's background scheduler silently died Aug 10 17:42 (API up, no jobs/logs for 34 h — restart fixed; overlay cron is hourly + 30-min quick-sync). Bogus TVDB episode ("Aflevering 7") drove a phantom Capture banner — unmonitored in Sonarr.
...
- **Movie `addedAt` corruption — REPAIRED 2026-08-05** (found by the first plex-audit run): the same Jul-20 event had stamped 272 movies with 2026-07-21. Fixed from **Radarr `movieFile.dateAdded`** (all 272 matched, 0 failures, locked); real top import day is 2022-03-16 (114 movies). Expect Leaving Soon - Movies to grow at the next rules run — the Jul-28 exclusions (445 vetoes) are intact, so only never-reviewed titles can enter.
