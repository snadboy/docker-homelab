...
**Episodes - JaD / J / D** (Plex TV collections; 2026-07-30 consolidation) — ONE shelf per viewing profile, replacing the New Series / New Season / Watching lifecycle trios AND the short-lived Upcoming Episodes shelf. Rule: **pool tag AND (a downloaded episode is unwatched OR the next episode airs within 7 days)** — combined household watch state (Plex admin viewCount ∪ Tautulli both users). The airing clause is what folds Upcoming Episodes in: caught-up shows resurface (posters carry the `S4E2 - Thu, Jul 30` banners) whenever an episode is imminent. Driven by Windmill `f/plex/jad_lifecycle_tagger` (source of truth; SSH-executes on arr) syncing Sonarr tags `jad-/j-/d-episodes` — resolved **by label, recreated when missing**, because Sonarr culls unused tags. Agregarr configs are the renamed-in-place Watching trio (Plex collections 62724/62734/62732, count-chip posters re-rendered by the tagger). Consequences to know: the old New-Series 28-day window is gone, so the shelf is the full unwatched backlog per pool (JaD 45 / J 2 / D 44 at cutover), and `windmill/scripts/plex_airing_soon_tagger.py` + Agregarr config 953887 + tag `airing-soon` were all retired. TV **Coming Soon** (118662) now sets `excludeFromCollections` to the three Episodes config ids — it keeps premieres 8-30d out + not-yet-in-library placeholders (trailers), never anything already on an Episodes shelf.
