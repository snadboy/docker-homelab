...
- **Available Episodes - JaD/J/D** (renamed-in-place configs 831979/835232/835245→835235; Plex rks 62724/62734/62732): pool tag AND ≥1 downloaded-unwatched episode. **Clean posters** — the red banner is suppressed by a new patched overlay context field `hasUnwatchedDownloaded` (Plex leafCount>viewedLeafCount) ANDed into banner templates 39/40's conditions. If the show's next episode airs ≤30d (necessarily undownloaded), the tagger sets a detail-page **Edition chip** `S4E3 - Aug 6` — store-and-restore in `~/scripts/upcoming-chips.json` on arr; **Leaving Soon pills always win the slot** (chip job skips Maintainerr group-1 members via localhost:6246 paged endpoint).
- **Upcoming Episodes** (config 960058, tag `upcoming-episodes`, autoPoster): monitored + has files + next **UNDOWNLOADED** episode ≤30d — regardless of watch state (2026-08-01: dual membership with Available is allowed; the caught-up-only rule was dropped by request). A show on both shelves keeps its **clean** poster (banner gate) + chip; banner posters mark "nothing to play until this date". "Undownloaded" matters: pre-released boxsets (Granite Harbour S3 on iPlayer) have future *air* dates but files on disk — the tagger checks per-episode `hasFile` (NOT series `nextAiring`), and an agregarr-patch context field `sonarrNextAiringDownloaded` (season fileCount≥totalCount) suppresses the banner fields for the same case.
- **Banner template 39** (only one enabled for TV): condition = 3 OR-sections (daysUntilRelease / daysUntilNextEpisode / daysUntilNextSeason, each ≤30 AND monitored AND `hasUnwatchedDownloaded eq false`). The next-episode section was missing until 2026-08-01 (lost in the PUT-wipe incident) — symptom: banners only on season premieres, never mid-season.
- ⚠️ **`applyOverlaysDuringSync` is OFF on all four shelf configs** (2026-08-01, deliberately): the collection-sync overlay path passes slim items with **no leafCount/viewedLeafCount**, so `hasUnwatchedDownloaded` computes false and banners get stamped onto Available shows. Only the nightly 03:00 `overlay-application` job (or manual `POST /api/v1/settings/jobs/overlay-application/run`) evaluates correctly — so a newly caught-up show waits up to a day for its banner. The full sweep is SLOW (~15-20s/item, ~4h for 749 shows; RT fetches) — one died mid-run on an unexplained 04:01 container restart, so verify completion, don't assume it.
