...
**Upcoming Episodes** (Plex TV collection, added 2026-07-30) — surfaces monitored shows whose next episode airs within 7 days (incl. fully-watched/caught-up ones, which otherwise vanish from Continue Watching while their poster banner goes unseen). Chain: Windmill `f/plex/airing_soon_tagger` (6-hourly; source of record `windmill/scripts/plex_airing_soon_tagger.py`) syncs Sonarr tag `airing-soon` → Agregarr sonarrtag config 953887 mirrors it into Plex collection 75580. Deliberately **no episode placeholders** (they'd fight Sonarr grabs, pollute On Deck, and skew Maintainerr's watched-count rules) — members are real shows; posters carry the `S4E2 - Thu, Jul 30` next-episode banners (Agregarr overlay templates, see agregarr skill). The tagger recreates the tag when Sonarr culls it (empty tags get deleted) and empties the Plex collection itself at zero members — Agregarr's sonarrtag sync skips empty tags and would strand the last member. Sonarr `nextAiring` skips already-aired-today episodes, so shelf dates can differ from the TMDB-fed banner date; both are correct. **Disjoint from Coming Soon** (2026-07-30): the TV Coming Soon config (118662) sets `excludeFromCollections: ["953887"]`, so premieres within 7 days appear only on Upcoming Episodes and migrate shelves automatically — Coming Soon is left with premieres 8-30d out plus not-yet-in-library placeholder shows (its unique trailer capability). NOTE 2026-07-30: fixed `u/dschless/sonarr_url` (pointed at retired `https://sonarr.isnadboy.com` → now `http://host-arr.isnadboy.com:8989`); this had silently broken `sonarr_tag_mirror` too — the last stale Traefik-era URL variable in Windmill.
